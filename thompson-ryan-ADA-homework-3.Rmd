...
boot_lm <- function(d, model, conf.level = 0.95, reps = 1000) {
  ...
  bs_ci_slope <- bootstrapped_lm %>% 
  summarize(
    beta1.boot = mean(slope),
    beta1.std.error.boot = sd(slope),
    beta1.perm.lower = quantile(slope, p_lower),
    beta1.perm.upper = quantile(slope, p_upper),
    beta0.boot = mean(intercept),
    beta1.std.error.boot = sd(intercept),
    beta0.perm.lower = quantile(intercept, p_lower),
    beta0.perm.upper = quantile(intercept, p_upper),
